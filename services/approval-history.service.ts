...
export async function getApprovalHistory(
  ...
  try {
    ...
    const response = await getBaseRecords(LARK_TABLES.APPROVAL_HISTORY, {
      ...
      sort: [
        {
          field_name: APPROVAL_HISTORY_FIELDS.timestamp,
          desc: true,
        },
      ],
    });
    ...
    return response.data.items.map((item: any) => ({
      ...
      employee_name: item.fields[APPROVAL_HISTORY_FIELDS.employee_name],
      action: item.fields[APPROVAL_HISTORY_FIELDS.action],
      approver_id: item.fields[APPROVAL_HISTORY_FIELDS.approver_id],
      approver_name: item.fields[APPROVAL_HISTORY_FIELDS.approver_name],
