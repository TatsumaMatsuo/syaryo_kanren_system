...
- 管理者による承認・却下処理
- **一括承認機能**: 複数申請の一括承認・却下
- **Lark Bot通知**: 承認・却下時に申請者へ自動通知
- 有効期限の自動通知（1週間前・期限切れ時）
- 許可証の発行・管理
- 退職時の論理削除機能
- **分析ダッシュボード**: 申請状況・期限切れ・承認履歴の可視化
...
### 2025-01-03: Lark通知機能・バグ修正

承認・却下時の申請者へのLark通知機能を追加し、複数のバグを修正しました。

**新機能**:
- **Lark Bot通知**: 承認・却下時に申請者へLarkメッセージを自動送信
  - 個別承認時の通知
  - 一括承認時の通知
  - 却下時の理由付き通知
  - 全書類承認時の許可証発行通知
...
**バグ修正**:
- 発行済許可証リストの重複表示を修正（record_idの重複除外）
- 一括承認時に削除済データが表示される問題を修正（`deleted_flag !== true`に変更）
- 検索ページの画像表示エラーを修正（`getFileUrl`ヘルパー関数追加）
- Vercelビルドエラーを修正（Buffer型、型プロパティの修正）
...
**新機能**:
- **分析ダッシュボード**: 申請状況、期限切れ状況、承認履歴のグラフ表示（recharts使用）
- **エクスポート機能**: 免許証・車検証・任意保険証・承認履歴をCSV/Excel形式でエクスポート
- **一括承認機能**: 複数の申請をまとめて承認・却下可能

**サイドメニューに追加**:
- 分析ダッシュボード（/admin/analytics）
- エクスポート（/admin/export）

### 2024-12-26: Phase 10 デプロイ・インフラ設定完了

本番環境へのデプロイ準備が完了しました。

**主な実装**:
- Sentry エラー監視の統合
- セキュリティヘッダーの設定（XSS対策、クリックジャッキング対策等）
- vercel.json の拡張（Cron、ヘッダー、関数設定）
- 本番環境設定ドキュメント（DEPLOYMENT.md）
